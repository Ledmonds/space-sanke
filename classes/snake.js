class Snake {
	...
	constructor(_snakehead_vector,_snake_color,_looping,_bearing) {
		...
		this.snake_looping = _looping; //Tracks if snake is in initial looping phase.
		this.snake_start_length = 5;
		this.snake_update_phase = false; //Ensures snake can't change direction twice in single draw call.
	...
	//Iterators
	iterateSnake() {
		//If looping, then loop snake in circle.
		if (this.snake_looping) this.loopSnake()

		//Iterate Snake in current bearing direction.
	...
	//Modifiers
	changeDirection(_bearing) {
		if (!this.snake_update_phase) {
			this.setBearing(_bearing);
